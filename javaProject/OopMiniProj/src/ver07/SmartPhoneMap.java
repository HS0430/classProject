...
//		numOfContact = 0;
	...
	private static SmartPhoneMap sp = new SmartPhoneMap();

	public static SmartPhoneMap getInstance() {

		if (sp == null) {
			sp = new SmartPhoneMap();
		}
...
//			companyContact.setCompany(newGroup);
	...
	void deleteContact() {
		...
		if (searchIndex < 0) {
			...
			for(int a=searchIndex; a<contacts.size(); a++) {
				contacts.put(a, contacts.get(a+1));
			}
			contacts.keySet().removeAll(Arrays.asList(contacts.size()-1, null));
		}
	}
