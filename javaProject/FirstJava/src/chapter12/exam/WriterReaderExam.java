...
//		④ 메모리스트와 파일 읽기 기능을 구현해봅시다
		try {
			int readData = 0;
			fin = new FileWriter(fileName + title +".txt", true);
			fin.write(contents+"\n");
			fin.flush();
			...
			fout = new FileReader(fileName+cTitle+".txt");
			while(true) {
				readData = fout.read(readContents);
				
				if(readData == -1) {
					break;
				}
				System.out.println(new String(readContents));
			}
